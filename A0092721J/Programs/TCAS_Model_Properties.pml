...
#define distance_computation_sanity(p) ((Aircraft[p]:otherAircraftInRA == 1) -> (Aircraft[p]:xDist <= RA_proportionality_const*Aircraft[p]:velocity && Aircraft[p]:yDist <= RA_proportionality_const*Aircraft[p]:velocity && Aircraft[p]:zDist <= RA_proportionality_const*Aircraft[p]:velocity))
/*Air space*/
...
bit moveCalledAfterCollision, moveCalled;
...
/*distance to other aircraft is computed only if it is in RA region or TA region*/
ltl dist_computation_check { [] distance_computation_sanity(collisionTestPID) };
